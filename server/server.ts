...
async function startServer(): Promise<Server> {
  ...
  // 全局错误处理
  // 注意：Express 靠「参数个数」识别错误处理中间件，必须是 4 个参数 (err, req, res, next)。
  // 若写成 3 个参数会被当作普通中间件 (req, res, next)，形参错位：res 实际收到 next 函数，
  // 调用 res.status() 即抛 `res.status is not a function`。
